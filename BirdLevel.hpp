...
/*
 * A BirdLevel is a scene augmented with some additional information which
...
//List of all levels:
extern Load< std::list< BirdLevel > > roll_levels;

struct BirdLevel : Scene {
	//Build from a scene:
	//  note: will throw on loading failure, or if certain critical objects don't appear
	BirdLevel(std::string const &scene_file);
	...
	BirdLevel(BirdLevel const &);
	// copy constructor actually just uses this = operator:
	BirdLevel &operator=(BirdLevel const &);
